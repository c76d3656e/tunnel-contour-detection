...
交互三维页用来看点云、当前剖面，以及按设计马蹄形做超欠挖对比；批量 `process` / `render` 仍走 Python。整体超挖/欠挖体积可在导出「桩号–体积」与各断面面积上继续积分。
...
设计轮廓是 **直墙圆弧拱马蹄形**（对齐 `docs/巷道设计断面.png` 的形状；图纸上的毫米数只作比例参考，可在左侧改全宽、直墙高、拱半径和位置）。马蹄形像拟合圆一样叠显。超欠挖 Δd 为实测轮廓到设计轮廓的法向距离：外侧为正（超挖），内侧为负（欠挖）。导出可出 docx 里的桩号–面积、桩号–体积、多断面轮廓、轮廓叠置和超欠挖对比图。
